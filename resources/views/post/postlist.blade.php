...
<div class="container">
  <div class="row justify-content-center">
    <div class="col-md-8">
      <div class="card">
        <div class="card-body">
          <form method="POST" action="{{ url('searchpost') }}">
          ...
          <div class="form-group">
            <div class="col-md-6">
              <a class="btn btn-primary btn-block" href="{{ url('post') }}">Add</a>
            </div>
          </div>
          <div class="form-group">
            <div class="col-md-6">
              <a class="btn btn-primary btn-block" href="{{ url('uploadcsv') }}">Upload</a>
            </div>
          </div>
          <form action="{{ url('downloadexcel') }}" method="GET" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
              <div class="col-md-6">
          ...
          <div class="container">
            <table class="table table-bordered">
              ...
              <tbody>
                ...
                <tr>
                  <td><a href="{{ url('postdetail', $list->title) }}">{{isset($list->title) ? $list->title : ''}}</a>
                  </td>
                  <td>{{isset($list->description) ? $list->description : ''}}</td>
                  <td>{{isset($list->name) ? $list->name : ''}}</td>
                  <td>{{isset($list->created_at) ? $list->created_at : ''}}</td>
                  <td><a href="{{ url('postupdate', $list->id) }}">Edit</a></td>
                  <td><a href="{{ url('deletepost', $list->id) }}" id="btnDeletePost">Delete</a></td>
                </tr>
                @endforeach
              </tbody>
            </table>
