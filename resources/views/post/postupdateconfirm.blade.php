            ...
            <div>
              <label class="switch">
                <input class="toggle-class" type="checkbox" disabled data-onstyle="success" data-offstyle="danger"
                  data-toggle="toggle" data-on="Active" data-off="InActive" {{ $pdata->status ? 'checked' : '' }}>
                <input type="hidden" id="postId" name="status" value="{{isset($pdata->status) ? $pdata->status : ''}}">
              </label>
            </div>
            <div class="confirmBtn">
              <button type="submit" class="btn btn-primary btn-lg">Update</button>
              <a href="{{ url('cancelupdate') }}"><input type="button" class="btn btn-outline-success btn-lg"
